...
# 资源路径（models/gazebo_models 含 ground_plane 等标准模型）
...
# -r: 启动后立即运行仿真，无需手动点击播放按钮
ign gazebo -r "$PROJECT_DIR/worlds/my_world.sdf" &
GZ_PID=$!

# DetachableJoint 默认初始附着；启动后持续发送一段时间 detach，避免"未碰撞也跟随"。
for _ in 1 2 3 4 5 6 7 8; do
  sleep 0.8
  ign topic -t /cs612/suction/detach -m ignition.msgs.Empty -p "unused: true" >/dev/null 2>&1 || true
done
