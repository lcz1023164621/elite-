...
def execution_stage(context: LaunchContext):
    ...
    rviz_launch = IncludeLaunchDescription(
            PythonLaunchDescriptionSource([elite_description, '/elite_description_ec64.launch.py']),
            condition=IfCondition(start_rviz)
        )

    urdf = os.path.join(get_package_share_directory('elite_description'), 'urdf', 'ec64_description_real.urdf')
