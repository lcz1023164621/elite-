...
def _load_yaml() -> dict[str, Any]:
    try:
        ...
        share = Path(get_package_share_directory("elite_moveit_ec612"))
...
class PlanningSceneSpawner(Node):
    ...
    def __init__(self) -> None:
        ...
        self._rect_size: list[float] = list(r.get("size_xyz", [0.14, 0.10, 0.06]))
        ...
        rc = r.get("center_xyz", [0.45, 0.0, 0.03])
        ...
        # RViz MotionPlanning 启动较慢时可能错过初次场景更新；周期性重发同一 revision。
        self.declare_parameter("scene_republish_period_sec", 5.0)
    ...
    def _tick(self) -> None:
        ...
        republish_period = max(1.0, float(self.get_parameter("scene_republish_period_sec").value))
        should_republish = (time.monotonic() - self._last_apply_time) >= republish_period
        if self._applied_revision is not None and self._applied_revision == rev and not should_republish:
            return
